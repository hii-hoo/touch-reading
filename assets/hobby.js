...
const vLength = ([x, y]) => sqrt(x * x + y * y)

const vAdd = (a, b) => [a[0] + b[0], a[1] + b[1]]

const vSub = (a, b) => [a[0] - b[0], a[1] - b[1]]

const vScale = (vec, s) => [vec[0] * s, vec[1] * s]

const vDistance = (from, to) => vLength(vSub(to, from))

const vRot = ([x, y], angle) => {
  let ca = cos(angle)
  let sa = sin(angle)
  return [x * ca - y * sa, x * sa + y * ca]
}

const vAngle = ([x, y]) => atan2(y, x)

const vAngleBetween = (v, w) =>
  atan2(w[1] * v[0] - w[0] * v[1], v[0] * w[0] + v[1] * w[1])
...
export function hobby(points, omega = 0.0) {
  ...
  for (let i = 1; i < n; i++) {
    ...
    C[i] = 1 / d[i]
    D[i] =
      (-1 * (2 * gamma[i] * d[i] + gamma[i + 1] * d[i - 1])) / (d[i - 1] * d[i])
  }
  ...
  for (let i = 0; i < n; i++) {
    res.push({ point: points[i], handleIn: c0[i], handleOut: c1[i] })
  }
